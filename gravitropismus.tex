...
Diese Fähigkeit der Pflanze, sich an der Schwerkraft zu orientieren, wird als Gravitropismus bezeichnet. Gravitropismus umfasst mehrere Schritte, die spezifisch nach einem Schema ablaufen.

Dazu gehört die Graviperzeption (Reorientierung im Schwerefeld), die Transduktion (Signalübermittlung des Reizes) und die gravitropische Krümmung, als Antwort auf den Reiz.
Dadurch kann die Organspitze die Wachstumsrichtung wieder aufnehmen zum Beispiel nach einem Regen oder einer Windaktion jeglicher Art.

Die primären Stellen der Wahrnehmung von der Veränderung der Schwerkraft befinden sich in der Wurzelhaube und in der Endodermis (innerste Zellschicht der Pflanzenrinde) des Sprosses.
Die Krümmungsantwort erfolgt danach in den Streckungszonen bei den jeweiligen Organen.

Bei der gravitropischen Stimulation wird Auxin (oder Gibberelline) erzeugt und an die Reaktionsstelle weitergeleitet, wo es dann das differenzielle Wachstum fördert.
...
Viele Pflanzen sind an die Erde festgebundene Organismen, die seit ihrer Keimung ihr ganzes Leben an einem Ort verbringen.

Dabei werden die Pflanzen durch Wurzeln an die Erde verankert, die dabei Wasser und Mineralionen aufnehmen, die wichtig für das Wachstum und die Entwicklung der Pflanzen sind.

Sprossen dagegen wachsen über der Erde, wo sie durch die Aufnahme des Lichts Photosynthese betreiben können. 
Daher müssen, während der Keimung und danach, Sprossen nach oben und Wurzeln nach unten wachsen, unabhängig von der ursprünglicher Lage des Samens auf der Erde.

Tatsächlich sind beide Organe (Wurzel und Spross) mit einer raffinierter Technik ausgestattet, die es ihnen ermöglicht die Schwerkraft zu spüren und sich nach ihr in einem richtigen Winkel zu richten.

Diese Bewegungsorientierung nach der Schwerkraft wird als Gravitropismus bezeichnet. Gravitropismus wurde vor zweihundert Jahren anerkannt (Knight, 1806). Seit dem wurde ihm viel Aufmerksamkeit gegeben.

Die meisten Prozesse, die jeder Phase der gravitropen Reaktion zugrunde liegen, bleiben jedoch schlecht
verstanden.

Dies liegt daran, dass die meisten der klassischen analytischen Methoden nicht ausreichen, um  solche hochentwickelte Prozesse zu analysieren.

Jüngste Fortschritte in den Disziplinen Genetik, Physiologie, Zellbiologie und Biochemie haben bedeutende Durchbrüche in Gravitropismus bei Spross und Wurzel ermöglicht. Dabei wird meist die \emph{Arabidopsis thaliana} als Model hergenommen.

Man erwartet, dass in den nächsten Jahren jeder Ablauf des Gravitropismus besser geklärt werden wird.

(Arabidopsis thaliana: A Model for the Study of Root and
Shoot Gravitropism)
...
Beim Gravitropismus (Geotropismus) werden drei Bewegungen unterschieden: Positiv gravitrop, negativ gravitrop und transversalgravitrop.
