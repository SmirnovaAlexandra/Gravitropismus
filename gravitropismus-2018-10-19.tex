...
Die Stärke aus den Statolithenamyloplasten kann man durch experimentelle Eingriffe, zum Beispiel durch Kühlung, verschwinden lassen \parencite[452]{Strasburger}.

Amyloplasten befinden sich in \emph{Statocysten}. Das sind Zellen, die die Graviperzeption ermöglichen. In diesen Zellen sind Statolithen zahlreich vorhanden. Kommen Statocysten in größeren Mengen vor, so bilden sie meist ein Gewebe, das als \emph{Statenchyme} bezeichnet wird.
...
Auxine sind Pflanzenhormone, die viele Funktionen besitzen. Dabei ist die Stimulierung der Zellstreckung (nur in niedriger Konzentration) und die Seiten- und Adventivwurzelbildung (?) besonders wichtig. Das dafür zuständige Auxin-Hormon ist die Indolessigsäure (IAA). (Auxine haben mehrere Formen blabla Bedarf an Erklärung)
